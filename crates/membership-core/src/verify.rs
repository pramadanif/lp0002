...
//! is invalid. There is deliberately no "returns false" path a caller could forget to check.
...
use crate::{ApprovalClaim, ApprovalWitness, VIEWING_PUBLIC_KEY_LEN};

/// Verifies a membership claim against its private witness and the account the caller presented.
///
...
/// # Panics
///
/// - the viewing key is not [`VIEWING_PUBLIC_KEY_LEN`] bytes;
/// - the witness does not control `bound_account_id` (**the live-account binding**, ADR-001 D4);
/// - the approver's `npk` is not a leaf under `claim.member_root`;
/// - `claim.claimed_nullifier` is not the nullifier this witness must produce.
...
/// Whether the witness's Merkle path carries this `npk` to `member_root`.
#[must_use]
pub fn proves_membership(
    ...
    root_from_path(&leaf, &path) == member_root
}
